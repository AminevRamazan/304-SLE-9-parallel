...
// Структура для данных, передаваемых потоку
struct ThreadData {
    std::vector<std::vector<double>>* A;
    int k, n;
    int start_row, end_row; // Диапазон строк, который будет обрабатывать поток
};

// Функция потока для инициализации матрицы
...
void initializeMatrix(std::vector<std::vector<double>>& A, int k, int n, int num_threads) {
    ...
    // Создаем потоки, каждому потоку назначаем диапазон строк
